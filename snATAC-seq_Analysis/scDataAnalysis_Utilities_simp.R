...
## calculate gene promoter and/or gene body accessibility
generate_gene_cisActivity <- function(gene_gtf, mtx, include_body = T,
  ...
  names(gene_ann)[1] = 'V1'
  gene_ann = subset(gene_ann, select = c(V1, V4, V5, V7, gene_name))
  names(gene_ann)[1:3] = c('chr', 'start', 'end')
  
  chrs = standardChromosomes(makeGRangesFromDataFrame(gene_ann))
  chrs = setdiff(chrs, excluded_chrs)
...
## add gene promoter and/or gene body accessibility into seurat object
addGeneActivityScore <- function(seurat.atac, gtf_file, include_gene_body = T, dist_to_tss = 2000,
                                 excluded_chrs = 'chrM'){
  atac.mtx = seurat.atac@assays$ATAC@counts
  rn = rownames(atac.mtx)
  rownames(atac.mtx) <- sapply(rn, function(x) unlist(strsplit(x, ','))[1])
  activity.matrix = generate_gene_cisActivity(gtf_file,
                                              atac.mtx,
                                              include_body = include_gene_body,
                                              excluded_chrs = excluded_chrs,
