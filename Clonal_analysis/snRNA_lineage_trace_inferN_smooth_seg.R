#install.packages("Clonalscope-main/", repos = NULL, type="source")
library(Clonalscope)
...
project_folder <- "/home/stat/jrong/nzhanglab/project/jrong/CPTCA_pHGG/"
args = commandArgs(trailingOnly=TRUE)

# One sample as example 
patient=args[1]
seed_snRNA=args[2]
snRNA_name = args[3] #"HGG_7316_1057_R2"
sample = args[4] #"7316_1057_reg2"
snRNA_name = strsplit(snRNA_name,",")[[1]]
  
start.time <- Sys.time()

# checking if having paired bulk data
if((is.na(sample)) | (nchar(sample) < 1)){ # no paired bulk sample for snRNA
  paired_bulk=FALSE
}else{
  paired_bulk = TRUE
}

dir_path = paste0(project_folder,"lineage_results_inferN_smooth_TumorOnly_all/",patient,"/")  # output or results folder
...
Cov_obj0 <- readRDS(paste0(project_folder,"inferN_smooth_results/smooth_seg_tumorOnly2/",seed_snRNA,"/Cov_obj.rds"))
...
tumor_metadata=tumor_anno@meta.data
rm(tumor_anno)
...
if(paired_bulk){# With paired bulk sample
  # read in bulk WGS from CNVkit output
...
# For each snRNA sample, perform lineage tracing
for(snRNA in snRNA_name){
  # load sample-specific WGS?
  print(paste0("Current Sample:",snRNA))
  dir_path = paste0(project_folder,"lineage_results_inferN_smooth_TumorOnly_allpatients/",patient,"/",snRNA,"/")
  if (!file.exists(dir_path)){dir.create(file.path(dir_path),recursive=T)}
  
  # extract corresponding sample's snRNA information
  mtx = snrna_obj@assays$RNA@counts[,snrna_obj$orig.ident == snRNA]
  ...
  #                    celltype=snrna_obj@active.ident[snrna_obj$orig.ident == snRNA])
  
  # celltype = data.frame(barcode = colnames(mtx),
  #                       celltype=snrna_obj$TumorNormal[colnames(mtx)])
  ...
  tumor_subclone_bc=intersect(rownames(celltype),rownames(tumor_metadata[tumor_metadata$orig.ident %in% snRNA,]))
  celltype[tumor_subclone_bc,"TumorAnno"] =  tumor_metadata[tumor_subclone_bc,"cell_state"]
  
  include='tumor'; # only clustering on tumor cells

  # Run the Clonalscope lineage tracing function
  set.seed(2022)
  ...
  Cov_obj=RunCovCluster(mtx=Input_filtered$mtx, barcodes=Input_filtered$barcodes,
                      ...
                      celltype0=NULL, var_pt=0.99, var_pt_ctrl=0.99, include='all',
                      ...
                      clustering0=clustering0, result0=result0,
                      clust_mode='all')
  ...
  clust_mode='all';
  clustering_barcodes=NULL; seed=200; clustering0=clustering0; result0=result0; verbose=FALSE;
  ...
  if( Est_read1==TRUE){# | !("deltas_allseg.rds" %in% list.files(paste0(dir_path,"rds")))){
    ...
    deltas_all=EstRegionCov(mtx=mtx, barcodes=barcodes, features=features, bed=bed, celltype0=celltype0, 
                            var_pt=var_pt, var_pt_ctrl=var_pt_ctrl, 
  ...
  print("Untill RunCovCluster Step:")
  print(time.taken)

  end.time2 <- Sys.time()
  time.taken2 <- end.time2 - start.time2
  print("RunCovCluster Step Only:")
  print(time.taken2)

  # save the clustering result related plots
  pdf(paste0(dir_path,paste0("/Clustering_Results_",".pdf")))
